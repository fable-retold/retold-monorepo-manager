/**
 * Manager-OperationBroadcaster (WebSocket pub/sub)
 *
 * The web transport's live operation stream. Clients connect to WS_PATH and receive JSON frames as
 * shelled operations run (started via the REST operation routes and bridged from ProcessRunner by
 * Manager-ProcessStreamBridge). Generic — no fork/ripple/retold specifics.
 *
...
class ManagerOperationBroadcaster extends libFableServiceProviderBase
{
	...
	/** Broadcast an arbitrary frame (used by the bulk-operation engine for its own frame vocabulary). */
	broadcast(pFrame)
	{
		this._broadcast(pFrame);
	}

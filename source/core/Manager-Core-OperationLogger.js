 ...
 * core — standalone TUI/web operations, the "all modules" shell scripts —
 * so there is a durable record of what happened and why, independent of the
 * server process or the browser.
 *
...
class OperationLogger
{
	 ...
	 * @param {string} pOptions.RepoRoot  Absolute path to the repo root (log lands here unless LogDir overrides).
	 * @param {string} [pOptions.LogDir]  Directory the log file lands in. Defaults to RepoRoot.
	 * @param {string} [pOptions.LogFilePrefix]  Log filename prefix. Defaults to 'Monorepo-Manager-Operations-'.
	...
	constructor(pOptions)
	{
		...
		this.logDir = tmpOptions.LogDir || tmpOptions.RepoRoot;
		this.logFilePrefix = tmpOptions.LogFilePrefix || 'Monorepo-Manager-Operations-';
	...
	_currentLogPath()
	{
		return libPath.join(this.logDir,
			this.logFilePrefix + todayStamp() + '.log');
	}

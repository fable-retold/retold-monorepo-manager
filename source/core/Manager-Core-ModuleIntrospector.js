/**
 * Monorepo Manager -- Module Introspector
 *
 ...
 *
 * This is the ORIGIN-ONLY variant of the introspector. The original
 * retold-manager modeled three git edges (Local <-> Fork <-> Upstream)
 * for a fork-based workflow; this port collapses the model to a single
 * remote (origin, by default) so the tool works against any monorepo
 * where each module tracks one canonical remote. Fork/upstream drift,
 * PR-mediated next-actions, and fetch-time freshness hints are gone.
 *
...
/**
 * Map a module's origin-relative git state to the single recommended next
 * action. This is the SERVER-SIDE single source of truth; the client renders
 * the code.
 *
 * With a single tracked remote (origin) there is exactly one edge: Local <->
 * Origin. The porcelain Ahead/Behind counts drive everything.
 *
 * pState (all optional, default 0/false):
 *   Dirty  - uncommitted / untracked working-tree changes
 *   Ahead  - committed commits not on origin (-> push)
 *   Behind - origin has commits local lacks (-> pull)
 *
 * Priority (most-blocking pending step first): commit -> pull -> push -> in-sync.
 */
function deriveNextAction(pState)
{
	let tmpS = pState || {};
	if (tmpS.Dirty) { return 'commit'; }
	if ((tmpS.Behind || 0) > 0) { return 'pull'; }
	if ((tmpS.Ahead  || 0) > 0) { return 'push'; }
	return 'in-sync';
}
...
class ModuleIntrospector
{
	 ...
	 * @param {string} [pOptions.RemoteName]     Git remote name to track (default 'origin').
	 * @param {string} [pOptions.DefaultBranch]  Branch fallback when porcelain can't parse one (default 'main').
	 */
	constructor(pOptions)
	{
		...
		// The single remote this tool tracks (origin by default) and the branch
		// fallback used when the porcelain header can't yield one.
		this.remoteName = tmpOptions.RemoteName || DEFAULT_REMOTE_NAME;
		this.defaultBranch = tmpOptions.DefaultBranch || DEFAULT_BRANCH;
	 ...
	 * ecosystem, with its local range and the currently-published npm
	 * version, and whether the range covers latest.
	 *
	...
	getEcosystemDepsSync(pName, pOptions)
	{
		...
		for (let i = 0; i < tmpDepNames.length; i++)
		{
			...
			if (!this._isEcosystemDependency(tmpDepName)) { continue; }
	...
	async getEcosystemDeps(pName, pOptions)
	{
		...
		let tmpDepNames = Object.keys(tmpAllDeps).sort();
		let tmpEcosystemDeps = tmpDepNames.filter((pDepName) => this._isEcosystemDependency(pDepName));
	...
	/**
	 * Is a dependency package name "in-ecosystem" for the stale-dep check?
	 *
	 * Prefers the injected manifest's own `isEcosystemDependency(name)` when
	 * present (ManifestLoader exposes configurable membership semantics);
	 * otherwise falls back to plain set-membership against the manifest's
	 * declared module names.
	 */
	_isEcosystemDependency(pDependencyName)
	{
		if (typeof this.manifest.isEcosystemDependency === 'function')
		{
			return this.manifest.isEcosystemDependency(pDependencyName);
		}
		return this._getEcosystemNameSet().has(pDependencyName);
	}
	...
	/**
	 * Quick summary of the working tree, origin-only. Reads porcelain (branch +
	 * local-vs-origin ahead/behind) and the origin remote URL, then derives the
	...
	getGitStatus(pName)
	{
		...
		// Resolve the tracked remote (origin by default) URL so the UI can
		// link to the module's canonical repo.
		let tmpOrigin = this._execGitSync('remote get-url ' + this.remoteName, tmpPath);
		tmpStatus.OriginUrl = (tmpOrigin || '').trim() || null;

		// Single recommended next action (server-side source of truth). The
		// porcelain Ahead/Behind is local vs its origin tracking ref.
		tmpStatus.NextAction = deriveNextAction(
			{
				Dirty:  tmpStatus.Dirty,
				Ahead:  tmpStatus.Ahead,
				Behind: tmpStatus.Behind,
			});
	 ...
	 * map keyed by module name with the parsed porcelain status decorated
	 * with LocalVersion / PackageName, the origin-only NextAction, and a
	 * categorized change rollup.
	 *
					...
					execAt('git status --porcelain -b', tmpEntry.AbsolutePath).then(function (pStatus)
						{
							...
							// Single origin-only next action from the porcelain edge.
							tmpParsed.NextAction = deriveNextAction(
								{
									Dirty:  tmpParsed.Dirty,
									Ahead:  tmpParsed.Ahead,
									Behind: tmpParsed.Behind,
								});

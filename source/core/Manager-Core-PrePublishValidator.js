/**
 * Retold Monorepo Manager -- Pre-Publish Validator
 *
 * Runs the read-only half of the publish step:
 ...
 *
 * Ecosystem membership generalization
 * ───────────────────────────────────
 * The upstream implementation baked "in-ecosystem" knowledge into a
 * project-specific introspector. Here that decision is an injected
 * dependency: a ManifestLoader-like object (preferred) or a bare
 * predicate. See `_resolveEcosystemMembership` — when neither is
 * supplied we fall back to the introspector's own view, so behavior is
 * identical to the original when the caller wires nothing extra.
 */
...
class PrePublishValidator
{
	 ...
	 * @param {object} pOptions.introspector                        Data-gathering seam (package.json / npm / git-log). NOT fork-aware.
	 * @param {object|function} [pOptions.ManifestLoader]           Ecosystem-membership source. Either a ManifestLoader-like
	 *                                                              object exposing `isEcosystemDependency(name)` (preferred),
	 *                                                              or `ecosystemNames` (Set/array) / `getAllModuleNames()`, or a
	 *                                                              bare predicate `(name) => boolean`.
	...
	constructor(pOptions)
	{
		...
		// Ecosystem-membership injection. Kept as its own resolved predicate so
		// the rest of the class never has to care which shape was passed in.
		this.manifestLoader = tmpOptions.ManifestLoader || null;
		this.isEcosystemDependency = this._resolveEcosystemMembership(this.manifestLoader);
	...
	/**
	 * Normalize the injected ecosystem-membership dependency into a single
	 * `(pDependencyName) => boolean` predicate.
	 *
	 * Resolution order:
	 *   1. `pManifestLoader.isEcosystemDependency(name)` — the ManifestLoader API.
	 *   2. `pManifestLoader.ecosystemNames` (Set or array).
	 *   3. `pManifestLoader.getAllModuleNames()` (array).
	 *   4. A bare predicate function passed directly.
	 *   5. null — no injected source; membership defers to the introspector's
	 *      own filtering (identical to the original behavior).
	 *
	 * @param {object|function|null} pManifestLoader
	 * @returns {(function(string): boolean)|null}
	 */
	_resolveEcosystemMembership(pManifestLoader)
	{
		if (!pManifestLoader)
		{
			return null;
		...
		// Bare predicate.
		if (typeof pManifestLoader === 'function')
		{
			return function (pName) { return !!pManifestLoader(pName); };
		}

		// Preferred: the ManifestLoader.isEcosystemDependency(name) contract.
		if (typeof pManifestLoader.isEcosystemDependency === 'function')
		{
			return function (pName) { return !!pManifestLoader.isEcosystemDependency(pName); };
		}

		// Fallback: a name set / array.
		let tmpNames = null;
		if (pManifestLoader.ecosystemNames)
		{
			tmpNames = pManifestLoader.ecosystemNames;
		}
		else if (typeof pManifestLoader.getAllModuleNames === 'function')
		{
			tmpNames = pManifestLoader.getAllModuleNames();
		}

		if (tmpNames)
		{
			// Support both Set and array.
			if (typeof tmpNames.has === 'function')
			{
				return function (pName) { return tmpNames.has(pName); };
			}
			if (Array.isArray(tmpNames))
			{
				let tmpSet = new Set(tmpNames);
				return function (pName) { return tmpSet.has(pName); };
			}
		}
	...
	/**
	 * Synchronous validation path. Preserves today's timing (one `npm view`
	 * per ecosystem dep, serially). Returns a complete report.
	 *
		...
		let tmpEcosystemDeps = this.introspector.getEcosystemDepsSync(pName,
			{
				...
				IsEcosystemDependency: this.isEcosystemDependency || undefined,
			});
	 ...
	 * dramatically faster for modules with many ecosystem dependencies.
	 */
